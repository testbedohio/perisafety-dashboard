...
// Robustly pull a JSON array out of a string that may contain prose, fences, etc.
function extractJSON(raw, label="data") {
  // 1. Strip markdown fences
  let s = raw.replace(/```json\s*/gi,"").replace(/```\s*/g,"").trim();

  // 2. Try parsing the whole string first (ideal case)
  try { return JSON.parse(s); } catch(_) {}

  // 3. Find the outermost [...] block by walking bracket depth
  let best = null;
